...
// `SemanticPreset` and `SemanticThresholds` moved into
// `DuplicationDetector` in Phase 5.4 of the audit-driven cleanup —
// they were `public` in an executable target, where the visibility
// had no effect. `ExpressibleByArgument` conformance for
// `SemanticPreset` lives in `CLIConformances.swift`.
...
public struct EmbeddingOptions: ParsableArguments {
    ...
    @Flag(
        name: .customLong("embedding-rerank-apted"),
        help: ArgumentHelp(visibility: .hidden),
    )
    public var rerankApted: Bool = false

    @Option(
        name: .customLong("embedding-apted-threshold"),
        help: ArgumentHelp(visibility: .hidden),
    )
