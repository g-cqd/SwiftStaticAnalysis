...
public struct ChangeDetector: Sendable {
    ...
    public func detectChanges(
        ...
        if configuration.parallelHashing {
            currentStates = await computeStatesParallel(for: currentFiles, previousState: previousState)
        } else {
            currentStates = computeStatesSequential(for: currentFiles, previousState: previousState)
        }
            ...
            for batchStart in stride(from: 0, to: files.count, by: batchSize) {
                ...
                for file in batch {
                    group.addTask {
                        ...
                        return (file, self.computeState(for: file))
                    }
                }
...
public enum FNV1a {
    ...
    private static let offsetBasis: UInt64 = 14695981039346656037
