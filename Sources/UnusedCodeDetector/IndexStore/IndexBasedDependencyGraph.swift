...
// MARK: - Index Based Dependency Graph
...
public final class IndexBasedDependencyGraph: @unchecked Sendable {
    ...
    private func buildEdges(from db: IndexStoreDB) {
        for filePath in analysisFiles {
            let occurrences = db.symbolOccurrences(inFilePath: filePath)
            for occurrence in occurrences {
                self.processOccurrence(occurrence, from: db)
            }
        }
    }
...
// MARK: - Index Graph Configuration
...
extension IndexBasedDependencyGraph {
    ...
    public func generateReport() -> IndexGraphReport {
        ...
        let externalNodes = nodes.values.filter { $0.isExternal }
