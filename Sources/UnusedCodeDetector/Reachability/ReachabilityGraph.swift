...
public struct DeclarationNode: Hashable, Sendable {
    ...
    public init(declaration: Declaration, isRoot: Bool = false, rootReason: RootReason? = nil) {
        self.id = "\(declaration.location.file):\(declaration.location.line):\(declaration.name)"
...
public final class ReachabilityGraph: @unchecked Sendable {
    ...
    @discardableResult
    public func addNode(_ declaration: Declaration, isRoot: Bool = false, rootReason: RootReason? = nil) -> DeclarationNode {
        lock.lock()
        defer { lock.unlock() }
...
// MARK: - Root Detection Configuration
